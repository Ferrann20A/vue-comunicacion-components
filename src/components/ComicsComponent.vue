<template>
  <div>
    ...
    <div v-if="comicFav" style="background-color:lightgreen">
        <h1>FAVORITO</h1>
        <p>{{comicFav.titulo}}</p>
        <img :src="comicFav.imagen"/>
    </div>
    <div v-for="(c,index) in comics" :key="c">
        <ComicComponent :comic="c" :index="index"
         v-on:seleccionarFavorito="seleccionarFavorito" v-on:eliminarComic="eliminarComic"/>
    </div>
  </div>
</template>
